...
class DemoOptLockingUsingShadowColumn
{
    ...
    throws SQLException
  {
    ...
    try
    {
      cstmt = conn.prepareCall( "{call opt_lock_shadowcol_demo.get_emp_details(?, ?, ?, ?)}" );
      ...
      System.out.println( "empno: " + empNo + ", name: " + empName + ", salary: " + salary + ", rowChangeIndicator: " + rowChangeIndicator );
    }
    finally
    {
  ...
  private static void _updateEmpInfo( Connection conn, int empNo, int newSalary, String newEmpName, long rowChangeIndicator )
    throws SQLException
  {
    CallableStatement cstmt = null;
    try
    {
      cstmt = conn.prepareCall( "{call opt_lock_shadowcol_demo.update_emp_info(?, ?, ?, ?, ?)}" );
      cstmt.setInt( 1, empNo  );
