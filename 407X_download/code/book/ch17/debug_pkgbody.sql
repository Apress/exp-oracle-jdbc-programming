...
package body debug as
  ...
  function get_debug_flag_desc( p_debug_flag in varchar2 )
  return varchar2 
  is
    l_debug_flag_desc varchar2(100 );
  begin
    case p_debug_flag 
      when DEBUG_OFF then
        l_debug_flag_desc := 'Debugging output is OFF for this session';
      when LOG_IN_TRACE_FILES_ONLY then
        l_debug_flag_desc := 'Debugging output in trace files only';
      when LOG_FOR_MIDTIER_ONLY then
        l_debug_flag_desc := 'Debugging output in middle tier only';
      when LOG_FOR_TRACE_FILES_AND_MTIER then
        l_debug_flag_desc := 'Debugging output in middle tier and trace files';
    end case;
    return l_debug_flag_desc;
  end;
  ...
  begin
    for c in ( select *
                ...
                where userid = user )
    loop
      if( c.debug_flag != DEBUG_OFF ) then
        ...
        then
          ...
          if( c.debug_flag = LOG_IN_TRACE_FILES_ONLY or
              c.debug_flag = LOG_FOR_TRACE_FILES_AND_MTIER ) then
            l_dummy := file_it( c.dir, c.filename, l_header || l_message );
          end if;
          if( c.debug_flag = LOG_FOR_MIDTIER_ONLY or
              c.debug_flag = LOG_FOR_TRACE_FILES_AND_MTIER ) then
            insert into debug_messages( message ) values ( l_message );
          end if;
        end if;
      end if;
    end loop;
  ...
  begin
    ...
    loop
      dbms_output.put_line( '---------------' || 
                            rpad( '-', length( p_user ), '-' ) );
      l_found := true;
      l_debug_flag_desc := get_debug_flag_desc( c.debug_flag ) ;
      dbms_output.put_line( 'DEBUG FLAG:           ' || 
                            l_debug_flag_desc );
  ...
  begin
    delete from debugTab 
     where userid = upper(p_user)
       and filename = nvl( p_file, filename )
       and dir = nvl( p_dir, dir );

    insert into debugTab(
      debug_flag, userid, modules, dir, filename, show_date, 
      date_format, name_length, show_session_id ) 
    values ( 
      p_debug_flag, p_user, p_modules, p_dir, p_file, p_show_date,
      p_date_format, p_name_len, p_show_sesid )
    returning
      debug_flag, userid, modules, dir, filename, show_date, 
      date_format, name_length, show_session_id
    into
      debugTab_rec.debug_flag, 
    ...
    l_debug_flag_desc := get_debug_flag_desc( debugTab_rec.debug_flag ) ;
    l_message := l_message || '     DEBUG FLAG: ' || 
      l_debug_flag_desc || chr(10);
  ...
  is
    l_cursor    sys_refcursor;
  begin
    l_cursor := get_debug_message;
    delete debug_messages;
    return l_cursor;
  end get_debug_message_flush;
